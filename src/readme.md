...
- **Static değişkenler SINIFA aittir (nesneye değil) Yani tüm nesnelerde ortaktır.**
- **Static değişkenler HAFIZADA TEK bir yerde durur**
- **Yani bir connection yapısı kullanacaksak her nesne oluşumunda yeniden connection oluşmasını istemiyorsak static olarka tanımlamamız mantıklıdır.**
#### **📌 Nerede Kullanılır?**
- **Sayaçlarda (bir şeyleri saymak için)**
- **Sabit değerlerde (pi sayısı gibi)**
- **Utility metodlarda (Math.random() gibi)**
- **Singleton pattern'de (sizin connection örneğiniz gibi)**
...
public class Ogrenci {
    // Static (ORTAK) değişken - tüm öğrenciler için aynı
    private static int ogrenciSayisi = 0;
    
    // Static olmayan (HER ÖĞRENCİYE ÖZEL) değişkenler
    private String ad;
    private int numara;
    
    public Ogrenci(String ad) {
        this.ad = ad;
        ogrenciSayisi++;  // Her yeni öğrenci oluşturulduğunda sayaç artar
        this.numara = ogrenciSayisi;  // Öğrenciye sıradaki numarayı ver
    }
...
public class Test {
    public static void main(String[] args) {
        // İlk öğrenciyi oluştur
        Ogrenci ahmet = new Ogrenci("Ahmet");
        ahmet.bilgileriGoster();
        // Çıktı:
        // Ad: Ahmet
        // Numara: 1
        // Toplam Öğrenci Sayısı: 1
        
        // İkinci öğrenciyi oluştur
        ...
        // İlk öğrencinin bilgilerini tekrar göster
        ahmet.bilgileriGoster();
        // Çıktı:
        // Ad: Ahmet
        // Numara: 1
        // Toplam Öğrenci Sayısı: 2  <- DİKKAT! Bu sayı arttı
    }
}
```
---
...
#### **📌 Evet adının static olması kafa karıştırıyor, ayrıca field gibi methodlar da static olabilir, mesela connection için yazılan methodlar :)**
...
**Ama son olarak daha teknik incelersek aslında composition yaptığımız sınıfın referansını (boş kutu)alırız**

Ayrıca neden direkt import etmiyoruz da bır sınıfı bu şekilde referansını alıyoruz bilmiyorum araştırırsın...
